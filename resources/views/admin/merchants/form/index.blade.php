...
<div class="col-lg-6">
    <div class="form-group">
        <label for="change_store" class="moto-widget-contact_form-label">Change Store</label>
            @if (!$readonly)
                <select name="change_store" required class="form-control w-100">
                    <option value="1" {{ old('change_store', $item->change_store ?? '') == true ? 'selected' : '' }}>Yes</option>
                    <option value="0" {{ old('change_store', $item->change_store ?? '') == false ? 'selected' : '' }}>No</option>
                </select>
            @else
                <input type="text" name="change_store" required="" class="form-control" value="{{ $item->status_name ? 'Yes' : 'No' }}" {{ $readonly ?? '' }}>
            @endif
    </div>
</div>
